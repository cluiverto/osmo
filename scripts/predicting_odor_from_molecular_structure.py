...
def goodscents():
    ...
    full_df['mol'] = full_df['IsomericSMILES'].apply(Chem.MolFromSmiles)
...
def create_all_features(df):
    ...
    return X_morgan, X_daylight


def create_mordred_features_cleaned(df):
    calc = Calculator(descriptors, ignore_3D=True)
    mordred_features = calc.pandas(df['mol'])

    mordred_features_clean = mordred_features.dropna(axis=1, thresh=int(0.8*len(df)))
    imputer = KNNImputer()
    mordred_imputed = imputer.fit_transform(mordred_features_clean)
    
    return mordred_imputed
